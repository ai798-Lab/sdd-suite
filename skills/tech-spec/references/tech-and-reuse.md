...
0. **先翻自己仓库,再往外看(最先做)**:在既有代码库上加东西时,先 grep 本仓库有没有能用的 helper / util / type / 既有模式,有就复用。往外找到的要读文档、要适配、要多养一个依赖;自己仓库里已经有的是零成本。**重写三个文件之外就有的东西,是 AI 写代码最常见的多余**,而且审不出来,因为它交的代码是对的,只是不该存在。全新项目从零起步时这条不适用,直接从第 1 条开始。

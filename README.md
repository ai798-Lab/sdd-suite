...
想法
 ...
 │                            自我审稿五视角:零上下文 / 样例真假 / 状态覆盖 / 最脆假设 / 自相矛盾
 │                            ⏸ 你 review、点头(审稿结论跟 spec 一起交到你手上)
 │
 ...
 │                            ⏸ 门 1:大白话讲清方案,你点头
 │                            tasks.md + plan.md(原子任务 + 批次编排 + worktree 执行清单)
 │                            ⏸ 门 2:讲清几批、哪些能同时开、几条并行线各动哪些文件,你点头
 │
 ④ 开发 ── 全部文件 ─► coding agent 按清单自动开 worktree 并行执行
                              写的时候守第 7 条最简阶梯(查表,不停不问)
                              每批合完出一张待删清单,清掉多余代码才开下一批
...
| 门 2 · 落地计划 | tasks + plan 出来、机器自检通过 | 总共几批几个任务、关键路径多长 / 哪些能同时开工 / 排了几条并行线各动哪些文件 / 每步做完你能亲自验到什么 | 点头计划 + 点头并行方案,才移交开发 |

产出能**直接喂并行执行**,而且 worktree 由 AI 自己开:tech-spec 会先探当前 coding agent 平台有没有 worktree 隔离、能不能并发派子 agent、能不能后台跑长任务,再给每个可并行批次排一份执行清单(worktree 目录 / 分支名 / 该分支允许改的文件集合 / 任务 / 完成判据 / 合并顺序),coding agent 照单建树、派活、验收、按序合并,批次间做 gate。**你不用记任何 `git worktree` 命令**;平台探不到并发能力就自动降级串行,产出不变,只是慢,并在门 2 明说降级原因。合并冲突一律停下来问你,不自作主张解决。移交前跑 `validate-plan.mjs` 机器自检:每条任务有文件路径 + 完成判据 + AC 回指、无 placeholder、同批 `[P]` 文件不重叠、有集成冒烟收尾、spec 每条 AC 都被覆盖。六项不过就打回重拆。
...
把 `spec.md` + 设计稿 + `plan.md` / `data-model.md` / `contracts/` / `tasks.md` 一起交给 coding agent(Claude Code 等),按批次照着实现。`plan.md` 里的 worktree 执行清单是给它照做的:建树、派活、跑判据、按序合并、收树,你只在合并冲突时被叫醒。spec 里的可验收 AC 就是验收清单。

这一段过去是盲区:前面所有的门都在动手之前,代码写出来之后就没人管了。方案定得再克制,实现时胖三倍你也看不见,因为你只 review 到 `plan.md` 为止。v0.8 补上两件事:

- **写的时候**:`CONSTITUTION.md` 第 7 条给了一条查表顺序(本仓库已有 → 标准库 → 平台原生 → 已装依赖 → 一行 → 才自己写)。它是查表不是论证,不写理由、不停下来问、不新增一道门。前六条都是重规矩,这一条必须轻才有用。
- **写完之后**:每批合并后除了集成冒烟,再过一遍整批 diff,出一张待删清单。冒烟只回答功能对不对,回答不了这些代码该不该存在;三条并行线各写一个功能重叠的工具函数,冒烟照样全绿。

## 套件构成

| skill | 干什么 | 产出 |
| --- | --- | --- |
| `discover-spec` ★ | **主角**。把想法逼成可落地的 spec,交人之前先自我审稿一遍 | `spec.md`(下方详解)+ 自我审稿结论 |
| `sdd-router` | 入口。判断你在第几步,送进对的门 | 路由到对的 skill |
| `tech-spec` | **主流程第三步**。设计稿之后一步做完技术方案与排期:自主调研选型 + 架构 + 数据模型 + 接口契约 + 四维自检 +(门 1)+ 原子任务与批次编排 + worktree 并行执行清单 +(门 2) | `plan.md` / `data-model.md` / `contracts/` / `tasks.md` |
| `gen-plan` | 旁挂的重排工具。方案不变、只想重新拆任务或换批次划分时单独用 | 重写 `tasks.md` + `plan.md` 执行计划段 |
| `sdd-orchestrate` | 总控:右尺寸判档 + 串成带门流程 | 一条 gated pipeline |
| `CONSTITUTION.md` | 常驻铁律:暴露而非假设 / 必停 review / 拒绝假大空 / 不擅自抽象 / 实现阶段最简阶梯 | 常驻约束 |
...
| 8 | 技术可行性初判 | 逐个功能一句话:靠什么实现 + 有没有现成轮子可用 + **难度量级** | 不写架构图、不做选型、不引具体定价(那是 tech-spec 的活);只用来挡下技术上做不了或做不起的功能 |
...
> 还配一个 `validate-spec.mjs` 脚本,机械检查:AC 六要素齐、有真实样例(拒绝假大空)、有页面清单、有技术可行性初判且逐功能给了难度量级、未决项盘点。

**交到你手上之前,它先自己审一遍。** 脚本只查结构,查不出样例是不是编的、状态漏没漏,所以 spec 落盘后还要过五个视角:**零上下文测试**(换一个没参与过对话的 agent 照着做,会卡在哪、会脑补什么)、**样例真假审计**(编的样例要么换成真的,要么标 `[ILLUSTRATIVE-EXAMPLE]`)、**状态与错误覆盖**(逐个功能点名过空 / 加载 / 成功 / 失败 / 无权限 / 并发重复提交)、**最脆假设**(挑出最经不起推敲的那条,配一个写码前最便宜的证伪动作)、**自相矛盾扫描**(前后文决策打架的逐条裁决)。发现的问题它自己改,改不动的列出来等你拍板,**审稿结论随 spec 一起交,不许只回一句「已自查」**。
...
- 流程:`discover-spec` 逼问 + 给建议 → 落成 `spec.md` 停下等你 review;点头(web/App 先去外部设计工具出稿)后 `tech-spec` 一步做完技术方案与计划,中间两道确认门,门 2 点头后交给 coding agent 按执行清单自动开 worktree 开干。

## 更新(已经装过旧版的)

v0.7 的变化:**`discover-spec` 多了一道自我审稿 pass**。spec 写完不直接端给你,先换个视角自己找茬:零上下文测试 / 样例真假审计 / 状态与错误覆盖 / 最脆假设 / 自相矛盾扫描,发现的问题自己改,改不动的列出来等你拍板,审稿结论随 spec 一起交。以前这里只有 `validate-spec.mjs` 查结构,查不了样例是不是编的、状态漏没漏。

v0.6 的变化:**并行执行编排并进 `tech-spec`**。以前 plan 排完只丢一句「每条 `[P]` 开一棵 worktree」,建树、派活、合并全靠你手敲;现在 tech-spec 先探平台能力(有没有 worktree 隔离 / 能不能并发派子 agent / 能不能后台跑长任务),再为每个可并行批次排出执行清单(目录 / 分支 / 文件集合 / 任务 / 判据 / 合并顺序),由 coding agent 自动跑完;探不到并发能力就降级串行并明说。门 2 同时确认并行方案。

v0.5 的变化:**排期职责从 `gen-plan` 并进 `tech-spec`**,一步出技术方案 + 计划,中间加两道用户确认门;`tech-spec` 的调研 pass 硬化成「自主选型、不给用户出技术选择题」;`gen-plan` 保留,降为独立重排工具。

按你的装法升级:

- **plugin 装法**:`/plugin update sdd-suite`,再 `/reload-plugins`(或重开会话)。
- **软链装法**:`cd sdd-suite && git pull`。这两版都没有新增 skill 目录,已软链的直接生效,不用补软链。
- **验证**:说"出技术方案"或"帮我排开发计划",都应该调起 `tech-spec`(而不是 gen-plan),在方案定稿后停下来用大白话跟你确认,并在计划里给出每条并行线的 worktree 清单。说"帮我写个 X 的 spec",落盘后交给你的应该是**一份 spec + 一张五视角审稿结论表**,不是光一份 spec。
...
**8 · 管到写代码那一刻,但那一段必须是轻的。**
前七条都在"动手之前"。可 coding agent 拿到 plan 之后会自己加东西:plan 里没写的抽象、跟仓库里已有函数重复的新函数、为几行代码装的新依赖。前七条一条也拦不住,而且你审不出来,因为它交的代码是对的,只是不该存在。所以加了第 7 条铁律和批次收尾的待删清单。**关键是它必须跟前面反着来**:前面都是要写理由、要过门、要等你点头的重规矩,这一条是查表,不写理由不停顿,轻到能当反射用。逐级认真斟酌反而让它变贵,就失去意义了。思路取自 [ponytail](https://github.com/DietrichGebert/ponytail)(MIT):它的对照实验显示,同样要求"少写代码",给一条有序阶梯比直接说"YAGNI、写一行"效果好近一倍,而且不会误伤输入校验和错误处理。
...
- **`discover-spec` 是被深度打磨过的那一个**;`tech-spec` 在 v0.5 才吸收排期职责、硬化调研 pass、加上两道确认门,v0.6 又接管了 worktree 并行编排(平台探底那段依赖各家 agent 的实际能力,可能探得不准),**实战验证还少**,用的时候多盯一眼产出,尤其盯它给的选型有没有真链接、真 star、真最近提交。
- **实现阶段那条阶梯是 v0.8 新加的,还没跑过足够多的真实项目**。它约束的是 coding agent 写代码时的选择,而它听不听、听多少,取决于它自己怎么读这段常驻 context。待删清单同理,能不能挑出多余的东西,取决于执行它的 agent 有没有真把整批 diff 读完。这两项都没有机器校验兜底,不像 tasks 那六项能跑脚本。
